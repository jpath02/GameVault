...
const games = [
  {
    ...
    category: "MOBA",
    platform: "PC",
    description: "Strategy team game where each player is competing in a five versus five match to destroy the enemy team’s base.",
    release: "2009-10-27",
 ...
 
  {
    ...
    platform: "Console, PC",
    description: "100 players competing in a battle royale style system where shooting mechanics are combined with a building aspect to see who is skilled enough to be the last man standing!",
    release: "2017-07-21",
...

  {
    ...
    platform: "Console, PC",
    description: "3D open-world where players aim to find resources and craft materials in order to survive and fully explore Minecraft.",
    release: "2009-05-17",
...

  {
    ...
    platform: "Console",
    description: "Chaotic racing game where players select their favourite characters within the world of Mario and compete head-to-head on who drives their kart the fastest!",
    release: "2014-05-29",
...

  {
    ...
    platform: "Console",
    description: "A basketball-simulated game to play as your favourite players that you watch in the NBA while customizing and unlocking different features to show your own unique flair!",
    release: "2024-09-06",
...

  {
    ...
    category: "RPG",
    platform: "Console",
    description: "Based in the Sinnoh region of the Pokémon world, you must venture through battles by catching and training your own Pokémon, slowly completing the Pokédex and working towards becoming the Pokémon master!",
    release: "2008-09-13",
...

  {
    ...
    category: "FPS",
    platform: "Console",
    description: " A juggernaut within the FPS genre, where players will combat with various types of enemies within the campaign mode using futuristic weapons and customizable loadouts to fight for their lives.",
    release: "2012-11-13",
...

  {
    ...
    platform: "Console",
    description: "You arrive in the Animal Crossing kingdom with the role of mayor, where you are now a focal point in the town, helping other animals with their day-to-day tasks and improving their livelihood here.",
    release: "2012-11-08",
...
let filterState = { //filters on Games page
  category: "All",     
  platform: "All",     
  sortBy: "price-asc", 
  maxPrice: 80,        
...
function parsePlatforms(p) { //turns "Console, PC" into ["console", "pc"] so its easier to check even if there are whitespaces or case mismatches
  return String(p)
    .split(",")
...
function initCategoryFilter() { //fills the category dropdown menu and event listener.
  ...
  sel.innerHTML = ordered.map((opt) => `<option value="${opt}">${opt}</option>`).join(""); //builds the options list once using HTML
...
function initPlatformFilter() { //same thing as category, but for platform dropdown now
...
function initSearchFilter() { //search box user input
...
if (btnClear) btnClear.onclick = () => { //resets state
  filterState = {
    category: "All",
    platform: "All",
    sortBy: "price-asc",
...
function showSection(id) { //hides all pages
...
function moveHero(dir) { //moves the carousel by +1 or -1.
  heroIndex =
    (heroIndex + dir + heroGames.length) % heroGames.length;
...
function setView(mode) { //grid/list toggle
...
function renderGames(list = games) { //renders list of cards
...
function viewGame(id) {
  ...
  const galleryImages = [g.gallery.main, ...g.gallery.thumbs, ...g.gallery.shots].slice(0, 9); //builds a flat array of 1 main, 4 thumbnails and 4 screenshot images
  ...
  document.querySelectorAll("#detailThumbs .thumb").forEach((t) => {
    t.addEventListener("click", () => {
      ...
      document.querySelectorAll("#detailThumbs .thumb").forEach((x) => x.classList.remove("active"));
...
function addCart(id) {
  ...
  if (item) item.qty++; //already in cart = +1
...
function updateCart() { //button shows total quantity of all items
...
function renderCart() {
  ...
  document.getElementById(
    "cartTotals"
  ).innerHTML = `<p>Subtotal: $${sub.toFixed(2)}<br>Tax (13%): $${tax.toFixed(
    2
...
function removeWish(id) { //removes an id from wishlist
